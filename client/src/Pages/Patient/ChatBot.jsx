...
const Chatbot = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (input.trim()) {
      ...
      try {
        const response = await axios.post('http://localhost:3000/groqMedBot', { prompt: input });
  ...
  return (
    <div>
      ...
      <motion.div
        ...
        className={`fixed bottom-0 right-0 w-full sm:w-[440px] h-[100vh] sm:h-[634px] bg-white p-6 rounded-t-lg sm:rounded-lg border border-gray-300 shadow-lg z-50 ${isOpen ? 'block' : 'hidden'}`}
      >
        {/* Chatbot Header */}
        ...
        >
          {messages.map((msg, index) => (
            <div key={index} className={`flex ${msg.sender === 'AI' ? 'justify-start' : 'justify-end'}`}>
              <div className={`flex items-center space-x-2 p-3 rounded-lg ${msg.sender === 'AI' ? 'bg-gray-100' : 'bg-black text-white'}`}>
        ...
        <form onSubmit={handleSubmit} className="flex items-center mt-4 space-x-2">
          <input
            type="text"
            className="flex-grow p-2 rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500"
